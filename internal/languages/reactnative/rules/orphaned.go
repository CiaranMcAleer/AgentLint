...
// This rule is intentionally conservative for single-file analysis.
...
func (r *UnusedFunctionRule) Check(ctx context.Context, node interface{}, config core.Config) *core.Result {
	...
	case *FunctionMetrics:
		// Skip exported functions - they may be used externally
		if n.IsExported {
			return nil
		}
		// Skip test functions
		if strings.HasPrefix(n.Name, "test") || strings.HasPrefix(n.Name, "Test") {
			return nil
		}
		// Skip lifecycle and hook functions
		if isReactLifecycleMethod(n.Name) {
			return nil
		}
		return nil
	}
...
func isReactLifecycleMethod(name string) bool {
